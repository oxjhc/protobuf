...
instance (EncodeWire a, KnownNat n) => GEncode (K1 i (Required n (Value a))) where
  gencode = gencode . first unRV

instance (EncodeWire (Enumeration a), Enum a, KnownNat n) => GEncode (K1 i (Required n (Enumeration a))) where
  gencode = gencode . first (fmap (fmap (fmap Enumeration)) . unRE) -- FML

instance (EncodeWire a, KnownNat n) => GEncode (K1 i (Optional n (Value a))) where
  gencode = gencode . first unOV

instance (EncodeWire (Enumeration a), Enum a, KnownNat n) => GEncode (K1 i (Optional n (Enumeration a))) where
  gencode = gencode . first (fmap (fmap (fmap Enumeration)) . unOE) -- FML
